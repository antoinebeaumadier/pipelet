...
const ValuesBlock: React.FC<ValuesBlockProps> = ({ block, onChange, allFields, inputData }) => {
    ...
    >
      ...
      >
        ...
        >
          <input
            type="checkbox"
            id={`valuesRecursive-${block.id}`}
            name="valuesRecursive"
            checked={block.config.valuesRecursive || false}
            onChange={(e) => {
              onChange({
                target: {
                  name: 'valuesRecursive',
          ...
          />
          <label
            htmlFor={`valuesRecursive-${block.id}`}
            style={{ fontSize: "12px" }}
          >
      ...
      >
        This block returns an array with all the values of an object. Select "Root object" to extract values from the entire item.
      </div>
    </div>
  );
};
