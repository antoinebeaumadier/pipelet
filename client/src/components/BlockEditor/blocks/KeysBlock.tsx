...
const KeysBlock: React.FC<KeysBlockProps> = ({ block, onChange, allFields, inputData }) => {
    ...
    >
      ...
      >
        ...
        >
          ...
          />
          <label
            htmlFor={`recursive-${block.id}`}
            style={{ fontSize: "12px" }}
          >
            Include nested keys recursively
          </label>
        </div>
      </div>
      ...
      >
        This block returns an array with all the keys of an object. 
      </div>
    </div>
  );
};
