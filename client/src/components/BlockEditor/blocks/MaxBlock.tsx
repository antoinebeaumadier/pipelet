...
const MaxBlock: React.FC<MaxBlockProps> = ({ block, onChange, allFields, inputData }) => {
    ...
    >
      ...
      >
        ...
        >
          <input
            type="checkbox"
            id={`maxRecursive-${block.id}`}
            name="maxRecursive"
            checked={block.config.maxRecursive || false}
            onChange={(e) => {
              onChange({
                target: {
                  name: 'maxRecursive',
      ...
      >
        This block returns the maximum value in a collection of data. Select "Root object" to find the maximum value in the entire item.
      </div>
    </div>
  );
};

export default MaxBlock; 